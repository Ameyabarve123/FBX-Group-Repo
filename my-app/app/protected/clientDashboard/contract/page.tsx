  ...
  return (
    <div className="flex-1 px-5 sm:px-8 py-8 space-y-5 overflow-auto min-h-screen" style={PAGE_BG}>
      ...
      <div className="pb-4 border-b border-white/[0.125] space-y-3">
        ...
        <div>
          <h1 className="text-white/100 text-2xl font-bold tracking-wide">Contract Details</h1>
          <p className="text-white/100 text-sm font-light tracking-wide mt-1">Review your agreement and fulfillment status</p>
        </div>
      </div>
      ...
      <div className={`rounded-xl bg-[#0d0b1e] border ${ACCENTS.teal.border} overflow-hidden`}>
        <div className="px-5 py-4 flex items-center justify-between border-b border-white/[0.08]">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-white/100 text-xs uppercase tracking-[0.18em] font-bold">Active Contract</h2>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="px-5 py-8 text-center text-white/100 text-xs tracking-[0.18em] uppercase">No contract on file — contact your FBX representative</div>
        ) : (
          <div className="divide-y divide-white/[0.06]">
              ...
              {[
                ...
                return (
                  <div key={label} className="relative p-5">
                    ...
                    <p className="text-[10px] uppercase tracking-[0.18em] text-white/100 font-bold mb-2">{label}</p>
            ...
            <div className="px-5 py-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] uppercase tracking-[0.18em] text-white/100 font-bold">Robot Fulfillment</p>
              </div>
              
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-[0.18em] text-white/100 font-bold">Progress</span>
              ...
              <p className="text-[10px] text-white/100">{shipped} of {allocated} robots shipped ({robotsRemaining} remaining)</p>
            </div>
            ...
            <div className="px-5 py-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] uppercase tracking-[0.18em] text-white/100 font-bold">Student Licenses</p>
              </div>
              
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-[0.18em] text-white/100 font-bold">Enrollment</span>
            ...
            {dbPlan.description && (
              <div className="px-5 py-4 space-y-2">
                <p className="text-[10px] uppercase tracking-[0.18em] text-white/100 font-bold">Contract Notes</p>
                <p className="text-white/100 text-sm leading-relaxed">{dbPlan.description}</p>
              </div>
            )}
            ...
            {dbPlan.created_at && (
              <div className="px-5 py-3 border-t border-white/[0.06]">
                <p className="text-[10px] text-white/100">Contract established {new Date(dbPlan.created_at).toLocaleDateString()}</p>
              </div>
            )}
          </div>
        )}
      ...
      <div className={`rounded-xl bg-[#0d0b1e] border ${ACCENTS.slate.border} overflow-hidden`}>
        <div className="px-5 py-4 flex items-center justify-between border-b border-white/[0.08]">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-white/100 text-xs uppercase tracking-[0.18em] font-bold">Invoice Logo</h2>
          </div>
        </div>
          ...
          <div className="flex items-start gap-4 flex-wrap">
            {hasEnterpriseLogo && logoPreviewUrl ? (
              ...
              <div className="flex-shrink-0 w-20 h-20 rounded-lg border border-white/[0.06] border-dashed bg-white/[0.02] flex items-center justify-center">
                <Image size={24} className="text-white/100" aria-hidden />
              </div>
            )}
            <div className="flex-1 min-w-0">
              <p className="text-white/100 text-sm">Your logo will appear on all invoices generated by FBX.</p>
              <p className="text-white/100 text-xs mt-1">PNG/JPG/GIF/WebP/SVG · max 15 MB · square recommended</p>
              {logoUploadError && (
